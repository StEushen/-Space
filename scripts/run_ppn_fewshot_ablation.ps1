...
$runCsv = "logs/ppn_fewshot_ablation_${Tag}.csv"
$summaryCsv = "logs/ppn_fewshot_ablation_${Tag}_summary.csv"
$rankCsv = "logs/ppn_fewshot_ablation_${Tag}_rank.csv"

"variant,ratio,seed,mse,mae" | Set-Content $runCsv -Encoding UTF8

$variants = @(
    @{ Name = "base"; Patch = 0; Gate = 0 },
    @{ Name = "patch_only"; Patch = 1; Gate = 0 },
    @{ Name = "gate_only"; Patch = 0; Gate = 1 },
    @{ Name = "patch_gate"; Patch = 1; Gate = 1 }
)
...
$rankRows = $summaryObjects |
    Sort-Object ratio, mse_mean, mae_mean, mse_std |
    Select-Object @{Name = "rank"; Expression = { 0 } }, variant, ratio, n, mse_mean, mse_std, mae_mean, mae_std

$rankFinal = @()
foreach ($ratioGroup in ($rankRows | Group-Object ratio)) {
    $idx = 1
